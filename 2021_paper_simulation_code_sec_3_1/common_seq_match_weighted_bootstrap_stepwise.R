...
for (t in 1 : n){
  ...
  # }
	...
	if (length(match_indic[match_indic == 0]) == 0 || t <= p || t <= min_t0_based_on_matching_pct){
		...
		ys_seen_previously = ys_to_date[1 : (t - 1)]
		w = indic_T[1 : (t - 1)]
		ys_seen_previously_adj_for_trt = lm.fit(as.matrix(w), ys_seen_previously)$residuals #summary(lm(ys_seen_previously ~ w))$residuals
		xs_seen_previously = data.matrix(xs_to_date[1 : (t - 1), ])
		#now adjust the x's for w --- THIS IS ILLEGAL FOR THE RANDOMIZATION TEST TO WORK!!!! SO DON'T DO IT!!!!
#		xs_seen_previously = (diag(t - 1) - w %*% t(w) / sum(w^2)) %*% xs_seen_previously
		
		# (e) we need to now run the stepwise procedure to get the weights
		weights = stepwise_weights(xs_seen_previously, ys_seen_previously_adj_for_trt)
...
#		}
		...
		for (b in 1 : NUM_BOOT_DSQD_DIST){
			two_xs  = xs_to_date[sample.int(t, 2), ]
