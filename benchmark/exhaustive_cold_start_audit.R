...
# Realistic clinical trial scale data generation
generate_data = function(n = 100, p = 7, family = "logistic", n_groups = 10) {
    ...
    if (family %in% c("logistic", "log-binomial", "identity-binomial", "glmm_logistic", "robust", "binomial-weighted")) {
        mu = 1 / (1 + exp(-eta))
        res$y = rbinom(n, 1, mu)
        if (family == "glmm_logistic") {
            re = rnorm(n_groups)[group_id]
            res$y = rbinom(n, 1, 1 / (1 + exp(-(eta + re))))
        }
...
run_bm = function(name, fit_fun, data, extra_args = list()) {
    ...
    if ("weights" %in% f_names) args_base$weights = rep(1.0, length(data$y))
...
# The exhaustive list of models supporting smart_cold_start
models = list(
    # Core GLMs
    list(name = "Logistic (IRLS)", fun = fast_logistic_regression_with_var_cpp, family = "logistic", args = list(optimization_alg = "irls")),
    list(name = "Poisson (IRLS)", fun = fast_poisson_regression_with_var_cpp, family = "poisson", args = list(optimization_alg = "irls")),
    ...
    list(name = "Identity Binomial Weighted", fun = fast_identity_binomial_regression_weighted_cpp, family = "identity-binomial"),
    list(name = "Logistic Weighted", fun = fast_logistic_regression_weighted_cpp, family = "binomial-weighted"),
    ...
    list(name = "Ordinal Weighted", fun = fast_ordinal_regression_weighted_cpp, family = "ordinal"),
