...
SeqDesignInferenceBaiAdjustedT = R6::R6Class("SeqDesignInferenceBaiAdjustedT",
  ...
  public = list(
    ...
    compute_treatment_estimate = function(){
      if (!private$convex_flag || private$KKstats$nRT <= 1 || private$KKstats$nRC <= 1){ #if er are not using the res in the test, only use the match pairs
  ...
  private = list(
    ...
    compute_halves = function(){
      ...
      pair_avg = do.call(rbind, lapply(split(1:nrow(X), private$KKstats$match_indic), 
