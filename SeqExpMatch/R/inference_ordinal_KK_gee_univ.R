...
#'
#' @description
#' Fits a Generalized Estimating Equations (GEE) model (via \code{multgee::ordgee})
...
#'
#' @details
#' This class requires the \pkg{multgee} package, which is listed under \code{Suggests}
#' and is not installed automatically with \pkg{SeqExpMatch}. Install it manually with
#' \code{install.packages("multgee")} before using this class.
#'
#' @export
#' @examples
#' set.seed(1)
...
#' infer <- SeqDesignInferenceOrdinalUnivKKGEE$new(seq_des, verbose = FALSE)
#' infer
#'
SeqDesignInferenceOrdinalUnivKKGEE = R6::R6Class("SeqDesignInferenceOrdinalUnivKKGEE",
