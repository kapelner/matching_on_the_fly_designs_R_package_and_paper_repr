...
#' )
#' seq_des <- SeqDesignCRD$new(n = nrow(x_dat), response_type = "survival", verbose = FALSE)
...
#'   c(1, 1, 0, 1, 0, 1, 1, 0)
#' )
#' infer <- SeqDesignInferenceSurvivalLogRank$new(seq_des, verbose = FALSE)
#' infer
#'
SeqDesignInferenceSurvivalLogRank = R6::R6Class("SeqDesignInferenceSurvivalLogRank",
	...
	public = list(
		...
		compute_mle_two_sided_pval_for_treatment_effect = function(delta = 0){
			...
			if (delta != 0){
				stop("Testing non-zero delta is not yet implemented for SeqDesignInferenceSurvivalLogRank.")
			}
		...
		compute_confidence_interval_rand = function(alpha = 0.05, nsim_exact_test = 501, pval_epsilon = 0.005, show_progress = TRUE){
			stop("Randomization confidence intervals are not supported for SeqDesignInferenceSurvivalLogRank due to inconsistent estimator units on the log-rank score scale.")
		}
	),
