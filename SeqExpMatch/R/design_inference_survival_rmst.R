...
#' seq_des_inf = DesignInferenceSurvivalRestrictedMeanDiff$
#'   new(seq_des)
#'
...
DesignInferenceSurvivalRestrictedMeanDiff = R6::R6Class("DesignInferenceSurvivalRestrictedMeanDiff",
	...
	public = list(
		...
		compute_confidence_interval_rand = function(alpha = 0.05, nsim_exact_test = 501, pval_epsilon = 0.005, show_progress = TRUE){
			stop("Randomization confidence intervals are not supported for DesignInferenceSurvivalRestrictedMeanDiff due to inconsistent estimator units on the transformed scale (estimates time difference, but randomization test searches for log-time ratio).")
		}
	),
