...
#' infer <- DesignInferenceIncidExactZhang$
#'   new(
#'   seq_des,
#'   verbose = FALSE
#' )
...
DesignInferenceIncidExactZhang = R6::R6Class("DesignInferenceIncidExactZhang",
	inherit = DesignInferenceIncidExactZhangAbstract,
	public = list(
	...
	private = list(
		...
		assert_supported_design = function(seq_des_obj){
			if (!is(seq_des_obj, "SeqDesignBernoulli")){
				stop(class(self)[1], " requires a completely randomized design (SeqDesignBernoulli). Use DesignInferenceIncidKKExactZhang for KK matching-on-the-fly designs.")
			}
		}
	)
)
