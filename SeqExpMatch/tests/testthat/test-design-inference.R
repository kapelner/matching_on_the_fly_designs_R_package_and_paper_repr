test_that("Inference works for continuous", {
	...
	# OLS
	inf_ols <- DesignInferenceContinMultOLS$new(des, verbose = FALSE)
...
test_that("Inference works for incidence", {
	...
	inf <- DesignInferenceIncidUnivLogRegr$new(des, verbose = FALSE)
	est <- inf$compute_treatment_estimate()
	expect_true(is.numeric(est))
})
...
test_that("Inference works for count", {
	...
	inf <- DesignInferenceCountUnivNegBinRegr$new(des, verbose = FALSE)
	est <- inf$compute_treatment_estimate()
	expect_true(is.numeric(est))

	inf_robust_pois <- DesignInferenceCountUnivRobustPoissonRegr$new(des, verbose = FALSE)
	est_robust_pois <- inf_robust_pois$compute_treatment_estimate()
	expect_true(is.numeric(est_robust_pois))

	inf_quasi_pois <- DesignInferenceCountUnivQuasiPoissonRegr$new(des, verbose = FALSE)
	est_quasi_pois <- inf_quasi_pois$compute_treatment_estimate()
	expect_true(is.numeric(est_quasi_pois))

	if (requireNamespace("glmmTMB", quietly = TRUE)) {
		inf_zinb <- DesignInferenceCountUnivZeroInflatedNegBinRegr$new(des, verbose = FALSE)
		est_zinb <- inf_zinb$compute_treatment_estimate()
		expect_true(is.numeric(est_zinb))
	}

	inf_hurdle_nb <- DesignInferenceCountUnivHurdleNegBinRegr$new(des, verbose = FALSE)
...
test_that("Inference works for survival", {
	...
	# KM Diff
	inf <- DesignInferenceSurvivalKMDiff$new(des, verbose = FALSE)
	est <- inf$compute_treatment_estimate()
	expect_true(is.numeric(est))

	# Log-rank
	inf_logrank <- DesignInferenceSurvivalLogRank$new(des, verbose = FALSE)
	est_logrank <- inf_logrank$compute_treatment_estimate()
	expect_true(is.numeric(est_logrank))

	# Cox PH
	inf_cox <- DesignInferenceSurvivalUniCoxPHRegr$new(des, verbose = FALSE)
	est_cox <- inf_cox$compute_treatment_estimate()
	expect_true(is.numeric(est_cox))

	# Stratified Cox PH
	inf_strat_cox <- DesignInferenceSurvivalUniStratCoxPHRegr$new(des, verbose = FALSE)
...
test_that("Inference works for ordinal partial proportional odds", {
	...
	inf_ppod <- DesignInferenceOrdinalPartialProportionalOdds$new(des, nonparallel = c("x"), verbose = FALSE)
...
test_that("Inference works for incidence KK Newcombe IVWC", {
	...
	inf <- DesignInferenceIncidUnivKKNewcombeRiskDiff$new(seq_des, verbose = FALSE)
	est <- inf$compute_treatment_estimate()
	expect_true(is.numeric(est))
