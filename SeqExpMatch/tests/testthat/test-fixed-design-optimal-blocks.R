test_that("FixedDesignOptimalBlocks is gated behind required libraries", {
	...
	if (!all_installed) {
		expect_error(
			FixedDesignOptimalBlocks$new(K = 2, n = 6, verbose = FALSE),
	...
	for (dist_name in c("euclidean", "sum_abs_diff", "mahal")) {
		des = FixedDesignOptimalBlocks$new(K = 2, dist = dist_name, n = n, verbose = FALSE)
		for (i in seq_len(n)) {
...
test_that("FixedDesignOptimalBlocks errors when roughly equal block sizes are infeasible", {
	...
	expect_error(
		FixedDesignOptimalBlocks$new(K = 7, n = 6, verbose = FALSE),
