...
public:
    ...
    double neg_log_likelihood(const VectorXd& params) const {
        ...
        for (int i = 0; i < m_n; ++i) {
            ...
            // cauchit(P(Y <= k)) = alpha_k - eta_i
            double p_upper = (yi_idx == m_K - 1) ? 1.0 : cauchit_inv(alpha[yi_idx] - eta[i]);
            double p_lower = (yi_idx == 0) ? 0.0 : cauchit_inv(alpha[yi_idx - 1] - eta[i]);
...
// [[Rcpp::export]]
List fast_ordinal_cauchit_regression_cpp(const Eigen::MatrixXd& X, const Eigen::VectorXd& y, int maxit = 100, double tol = 1e-6) {
...
List fast_ordinal_cauchit_regression_with_var_cpp(const Eigen::MatrixXd& X, const Eigen::VectorXd& y) {
    List res = fast_ordinal_cauchit_regression_cpp(X, y);
