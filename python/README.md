...
`EDI/src/*.cpp` model-fitting kernels the R package `EDI` uses, built under
an `EDI_CORE_ONLY` preprocessor guard that swaps out `RcppEigen`/`Rmath` for
a vanilla `Eigen` + `LBFGSpp`, both fetched directly from their own upstream
repositories (see `CMakeLists.txt`). Nothing under `python/` is a copy of a
`EDI/src/*.cpp` or `*.h` file — the compiled extension `#include`s them
directly from the R package's own source tree.
