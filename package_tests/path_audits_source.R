...
audit_classes = list(
  ...
  # ── ORDINAL ───────────────────────────────────────────────────────────────────
  list(name="InferenceOrdinalKKGEE",                    section="Ordinal", resp="ord", kk=TRUE,  types="wald", skip_asymp=FALSE, skip_ci=FALSE, skip_boot=FALSE, skip_bbt=FALSE, jack=TRUE,  skip_rand=FALSE, rand_resp="ordinal", skip_rpv=FALSE, skip_rci=FALSE, rci_resp="", pboot=NA, slow_methods=c("compute_bootstrap_confidence_interval_studentized", "compute_bootstrap_two_sided_pval_studentized", "compute_rand_two_sided_pval(delta=0.5)"), notes="InferenceAsymp; compute_treatment_estimate_during_randomization_inference overridden to reuse ordLORgee fit (2026-07-28 fix: inherited mixin default routed through geeglm+binomial, which errors on >2-level responses and always returned NA)"),
