...
script_path = sub("^--file=", "", grep("^--file=", commandArgs(FALSE), value = TRUE)[1] %||% "package_tests/run_public_argument_combination_nightly.R")
...
nightly_target_filter = function(value) {
	if (is.null(value) || !nzchar(value)) return(default_smoke_target_filter)
	if (identical(value, "ALL")) return(NULL)
	value
}

nightly_fixture_ids = function(value) {
...
main = function() {
	...
	target_filter = nightly_target_filter(if (length(args) >= 1L) args[1L] else NULL)
	fixture_ids = nightly_fixture_ids(if (length(args) >= 2L) args[2L] else NULL)
	class_filter = if (length(args) >= 3L && nzchar(args[3L])) args[3L] else NULL
	method_filter = if (length(args) >= 4L && nzchar(args[4L])) args[4L] else NULL
	timeout_sec = if (length(args) >= 5L && nzchar(args[5L])) as.numeric(args[5L]) else 60
	...
	gates = check_public_argument_combination_quality_gates("report")
	message("Public argument combination nightly rows: cases=", nrow(cases), ", results=", nrow(results), ", coverage=", nrow(analysis$coverage), ", integrated_cases=", nrow(integrated$cases), ", active_hard_gates=", gates$summary$active_hard_rows[1L])
