...
#pragma omp parallel for schedule(dynamic)
	for (int b = 0; b < nsim; ++b) {
		...
		Eigen::VectorXi m_vec = m_mat.col(b);

		int m = m_vec.maxCoeff();
		...
		if (m > 0) {
			...
			for (int match_id = 1; match_id <= m; match_id++) {
				...
				for (int i = 0; i < n; i++) {
					if (m_vec[i] == match_id) {
						if (w[i] == 1) { yT = y[i]; foundT = true; }
						else { yC = y[i]; foundC = true; }
		...
		if (nRT > 0 && nRC > 0) {
			...
			if (nRT > 1 && nRC > 1 && (nRT + nRC) > 2) {
				...
				for (int i = 0; i < n; i++) {
					if (m_vec[i] == 0) {
						if (w[i] == 1) sq_diff_T += (y[i] - mean_T) * (y[i] - mean_T);
