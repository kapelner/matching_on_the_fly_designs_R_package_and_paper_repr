...
List fast_ordinal_cloglog_regression_with_var_cpp(const Eigen::MatrixXd& X, 
                                                   ...
                                                   Nullable<NumericVector> fixed_values = R_NilValue,
                                                   Rcpp::Nullable<Rcpp::NumericMatrix> warm_start_fisher_info = R_NilValue) {
    List res = fast_ordinal_cloglog_regression_cpp(X, y, warm_start_params, smart_start, 100, 1e-6, optimization_alg, fixed_idx, fixed_values, warm_start_fisher_info);
