...
// [[Rcpp::export]]
NumericVector test_ols_warm_start_beta_cpp(const Eigen::MatrixXd& X, const Eigen::VectorXd& y) {
	return wrap(ols_warm_start_beta(X, y));
}

// [[Rcpp::export]]
NumericVector test_ols_warm_start_beta_on_log1p_cpp(const Eigen::MatrixXd& X, const Eigen::VectorXd& y) {
	return wrap(ols_warm_start_beta_on_log1p(X, y));
}

// [[Rcpp::export]]
NumericVector test_finalize_warm_start_beta_cpp(const Eigen::VectorXd& smart_start,
	...
	return wrap(finalize_warm_start_beta(smart_start, legacy_start, fixed_spec, use_smart));
}
...
List test_ordinal_start_cpp(const Eigen::MatrixXd& X,
	...
	OrdinalStart warm_start_params = ordinal_start_from_ols(X, y, ordinal_link);
	return List::create(
