...
InferenceIncidBinomialIdentityRiskDiff = R6::R6Class("InferenceIncidBinomialIdentityRiskDiff",
	...
	private = list(
		...
		compute_treatment_estimate_during_randomization_inference = function(estimate_only = TRUE){
			...
			res = fast_identity_binomial_regression_cpp(X = Xmm, y = as.numeric(private$y))
			...
			if (private$include_covariates) {
				...
				attempt = private$fit_with_hardened_qr_column_dropping(
					...
					fit_fun = function(X_fit, j_treat){
						if (estimate_only) {
							res = fast_identity_binomial_regression_cpp(X_fit, private$y)
							list(b = res$b, ssq_b_2 = NA_real_, j_treat = j_treat)
						} else {
							res = fast_identity_binomial_regression_with_var_cpp(X_fit, private$y, j = j_treat)
							res$j_treat = j_treat
							res
						}
					},
				...
				if (estimate_only) {
					res = fast_identity_binomial_regression_cpp(Xmm, private$y)
					list(b = res$b, ssq_b_2 = NA_real_)
				} else {
					fast_identity_binomial_regression_with_var_cpp(Xmm, private$y)
				}
			}
		}
	)
