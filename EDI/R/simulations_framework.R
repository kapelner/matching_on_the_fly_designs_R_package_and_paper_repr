...
SimulationFramework = R6::R6Class("SimulationFramework",
  ...
  public = list(
    ...
    #'
    #' @param observational_dgp Optional function for a fully custom
    #'   \emph{observational-study} DGP. Signature:
    ...
    initialize = function(
      ...
      if (!is.null(observational_dgp)) {
        if (!is.null(custom_replication_data_generator) || !is.null(custom_apply_treatment_and_noise))
          stop("observational_dgp cannot be combined with custom_replication_data_generator or custom_apply_treatment_and_noise")
        if (!is.function(observational_dgp))
          stop("observational_dgp must be a function")
        if (all(betaT_values != 0))
          warning("observational_dgp is set; betaT is ignored as the true estimand comes from the DGP function")
      }
      ...
      private$observational_dgp = observational_dgp
    ...
    run = function() {
      ...
      for (cell_idx in seq_len(n_cells)) {
        ...
        cs = list(
          ...
          observational_dgp = private$observational_dgp,
    ...
    .run_single_replication_in_worker = function(rep_i, state, progress_cb = NULL, is_forked = FALSE) {
      ...
      for (di in seq_along(state$design_classes)) {
        ...
        des_obj = tryCatch({
          ...
          if (!is.null(state$observational_dgp)) {
            ...
            if (grepl("SeqOneByOne", design_gen$classname, fixed = FALSE))
              stop("observational_dgp requires a fixed design class; '",
                   design_gen$classname, "' is a sequential design")
            dgp_fn = state$observational_dgp
            ...
            if (!is.list(obs_out) || is.null(obs_out$X) || is.null(obs_out$w) || is.null(obs_out$y))
              stop("observational_dgp must return a list with 'X', 'w', and 'y'")
            if (!is.data.frame(obs_out$X)) obs_out$X = as.data.frame(obs_out$X)
            if (nrow(obs_out$X) != state$n)
              stop("observational_dgp returned X with ", nrow(obs_out$X), " rows; expected ", state$n)
        ...
        if (is.null(des_obj)) next
        # Per-design true estimand: custom_true_estimand (Mode 2) or observational_dgp$true_estimand (Mode 3)
        ...
        for (ii in seq_along(state$inference_classes)) {
          ...
          te = if (!is.null(state$custom_true_estimand) || !is.null(state$observational_dgp)) {
          ...
          local_record = function(type, ci, pval) {
            ...
            sim_mode = if (!is.null(state$observational_dgp)) {
              "observational"
            } else {
              parts = c(
