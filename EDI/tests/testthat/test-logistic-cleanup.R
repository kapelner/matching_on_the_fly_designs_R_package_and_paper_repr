...
test_that("fast_logistic_regression_cpp works without smart_start", {
  set.seed(123)
  n <- 100
  p <- 3
  ...
  # This should now work without smart_start argument
  res <- fast_logistic_regression_cpp(X, y)
  expect_equal(length(res$b), p)
  expect_true(res$iterations > 0)
})

test_that("fast_logistic_regression_with_var_cpp works without smart_start", {
  set.seed(123)
  n <- 100
  p <- 3
  X <- matrix(rnorm(n * p), n, p)
